...
def plot_history(history, model_name, Fold_Num):
    ...
    plt.savefig(f'Results/{model_name}/Fold_{Fold_Num}/{history_train_param_1_name}_Plot_Fold_{Fold_Num}.png')
    plt.tight_layout()
    plt.show()
    plt.close()
    ...
    plt.xlabel('Epoch', fontsize=14)
    plt.legend(['Train', 'Val'], loc='upper right')
    plt.savefig(f'Results/{model_name}/Fold_{Fold_Num}/{history_train_param_2_name}_Plot_Fold_{Fold_Num}.png')
    plt.tight_layout()
    plt.show()
    plt.close()
