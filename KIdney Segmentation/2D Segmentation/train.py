...
for i in range(start_fold, (end_fold + 1)):
    ...
    if os.path.exists(f'Results/{model_name}/Fold_{i}/'+model_name+'_'+str(imwidth)+'_Fold_'+str(i)+'.keras') and (load_weights == True) and (encoder_trainable == False):
        print('Loading PreTrained Weights...')
        model.load_weights(f'Results/{model_name}/Fold_{i}/'+model_name+'_'+str(imwidth)+'_Fold_'+str(i)+'.keras')
    if encoder_trainable == True:
        if os.path.exists(f'Results/{model_name}/Fold_{i}/'+model_name+'_'+str(imwidth)+'_Fold_'+str(i)+'.keras'):
    ...
    callbacks = [EarlyStopping(monitor=monitor_param,
                               ...
                               mode=patience_mode),
                 ModelCheckpoint(f'Results/{model_name}/Fold_{i}/'+model_name+'_'+str(imwidth)+'_Fold_'+str(i)+'.keras',
                                 verbose=1,
    ...
    if save_history:
        ...
        history_path = f'Results/{model_name}/Fold_{i}'
        ...
        plot_history(history_dict, model_name, i)
